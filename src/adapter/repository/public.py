        ...
        with self.session() as session:
            ...
            for key, value in filters.items():
                ...
                if key in {"name", "email"}:
                    conditions.append(getattr(self.model, key).ilike(f"%{value}%"))
                else:
            ...
            if period and isinstance(period, dict):
                ...
                if hasattr(self.model, "created_at"):
                    if start:
                        conditions.append(getattr(self.model, "created_at") >= start)
                    if end:
                        conditions.append(getattr(self.model, "created_at") <= end)
